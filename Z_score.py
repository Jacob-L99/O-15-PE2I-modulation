...
def SD_corrected(transformed_K_1, frame, wat, normal_data):
    ...
    if frame=="real" and wat== 'wat':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1.txt')
    elif  frame=="ref" and wat=='wat':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_ref.txt')
    elif frame=='real' and wat=='wat1':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_1.txt')
    elif frame=='ref' and wat=='wat1':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_1_ref.txt')
    elif frame=='real' and wat=='wat2':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_2.txt')
    elif frame=='ref' and wat=='wat2':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_2_ref.txt')
    elif frame=='real' and wat=='wat1_2':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_3.txt')
    elif frame=='ref' and wat=='wat1_2':
        file_path = os.path.join(normal_data, 'flow_region_mean_std_k1_3_ref.txt')
...
def SD_corrected_park(R_I_reshape_list, tracer, normal_data):
    ...
    # Let's read the data directly from the text file named "Mean perfusion with slope and intersect"
    if tracer == "C-11":
        file_path = os.path.join(normal_data, '48_region_mean_std_R_I_C.txt')
    else:
        file_path = os.path.join(normal_data, '48_region_mean_std_R_I.txt')
    ...
    z_scores_flow=[z_cerebellum_dex, z_cerebellum_sin, z_anterior_dex, z_anterior_sin, z_middle_dex,
                   z_middle_sin, z_posterior_dex, z_posterior_sin]
    # print("hej=",z_cerebellum_dex)
    # np.save('Z_brain.npy', Z_brain)
    # np.save('z_scores_flow.npy', z_scores_flow)
    # np.save('means_list.npy', means_list)
